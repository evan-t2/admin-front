...
async function fetchAdmins() {
    loading.value = true
    try { const res: any = await adminApi.getList(); admins.value = res.data.content } catch {}
    finally { loading.value = false }
}

async function handleDelete(adminIdx: number) {
    await ElMessageBox.confirm('삭제하시겠습니까?', '확인')
    try { await adminApi.delete(adminIdx); ElMessage.success('삭제되었습니다.'); fetchAdmins() } catch {}
}

function getTypeName(type: number) { if (type >= 100) return '최고마스터'; if (type >= 50) return '마스터'; return '일반' }

onMounted(fetchAdmins)
</script>

<template>
    <PageContainer title="관리자 관리">
        <template #actions><el-button type="primary" @click="router.push('/settings/admins/create')">관리자 추가</el-button></template>
        <el-table :data="admins" v-loading="loading" border>
            ...
            <el-table-column label="등급" width="100" align="center">
                <template #default="{ row }"><el-tag :type="row.adminType >= 50 ? 'danger' : 'info'" size="small">{{ getTypeName(row.adminType) }}</el-tag></template>
            </el-table-column>
            <el-table-column label="상태" width="80" align="center">
                <template #default="{ row }"><el-tag :type="row.adminStats === 'y' ? 'success' : 'info'" size="small">{{ row.adminStats === 'y' ? '활성' : '비활성' }}</el-tag></template>
            </el-table-column>
