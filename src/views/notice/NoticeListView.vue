...
async function fetchNotices() {
    loading.value = true
    try {
        const res: any = await noticeApi.getList()
        notices.value = res.data
    } catch {
        ElMessage.error('공지사항 목록을 불러오지 못했습니다.')
    } finally {
        loading.value = false
    }
}
...
function handleSelectionChange(rows: Notice[]) {
    selectedIds.value = rows.map((r) => r.index)
}
...
async function handleBatchStatus(isShowing: boolean) {
    if (selectedIds.value.length === 0) {
        ElMessage.warning('선택된 항목이 없습니다.')
        return
    }
    try {
    ...
    <PageContainer>
        ...
        <el-table :data="notices" v-loading="loading" @selection-change="handleSelectionChange" border>
            ...
            <el-table-column label="관리" width="100" align="center">
                <template #default="{ row }">
                    <el-button type="danger" size="small" text @click="handleDelete(row.index)">삭제</el-button>
                </template>
            </el-table-column>
        </el-table>
    </PageContainer>
